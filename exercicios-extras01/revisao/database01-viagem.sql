create database aula_ex_viagem
GO
use aula_ex_viagem
...
SELECT c.marca, c.modelo, SUM(v.distanciaPercorrida) AS distancia_total
...
SELECT e.nome, c.marca, c.modelo, COUNT(v.idCarro) AS qtd_viagem
FROM empresa e, carro c, viagem v
WHERE c.idEmpresa = e.id
   AND v.idCarro = c.id
GROUP BY e.nome, c.marca, c.modelo
ORDER BY e.nome ASC, COUNT(v.idCarro) DESC
